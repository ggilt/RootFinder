...
public class tab3 extends JFrame implements ActionListener {
    ...
    double x1, x2, startP1, startP2, root;
    int decimalPl, maxIt;
    String methodName;
    ...
    private void specifyPanel() {
        ...
        JLabel l_function1 = new JLabel("f(x)=ln(x+");
        JLabel l_function2 = new JLabel(")+");
        ...
        if ("find".equals(e.getActionCommand())) {
            try // tries to parse the data user has input         
            {
                ...
                if (methodName.equals("Newton-Raphson")) {
                    solve(methodName, x1, x2, startP1, decimalPl, maxIt, -1);
                } else {
                    startP2 = Double.parseDouble(startP2Field.getText());
                    solve(methodName, x1, x2, startP1, decimalPl, maxIt, startP2);
                }
    ...
    private void solve(String methodName, double a, double b, double sP1, int dp, int it, double sP2) {
        if (methodName.equals("Bisection")) {
            BisectionMethod BM = new BisectionMethod(a, b, sP1, sP2, dp, it);
            double[][] result = BM.getResult();
            root = BM.getRoot();
            displayBisectionData(result, root);
        }
        if (methodName.equals("Newton-Raphson")) {
            NewtonRhapsonMethod NM = new NewtonRhapsonMethod(a, b, sP1, dp, it);
            double[][] result = NM.getResult();
            root = NM.getRoot();
            displayNewtonData(result, root);
        }
        if (methodName.equals("Secant")) {
            SecantMethod SM = new SecantMethod(a, b, sP1, sP2, dp, it);
    ...
    private void displayBisectionData(double[][] result, double root) {
        ...
        JFreeChart chart = ChartFactory.createXYLineChart(
                "Graph for the Log function, using Bisection Method",
                "x",
                "f(x)",
                dataset,
    ...
    private void displayNewtonData(double[][] result, double root) {
        ...
        dataset.addSeries(rootPlot);
        JFreeChart chart = ChartFactory.createXYLineChart(
                "Graph for the Log function, using Newton-Raphson Method",
                "x",
                "f(x)",
                dataset,
                PlotOrientation.VERTICAL,
    ...
    private void displaySecantData(double[][] result, double root) {
        ...
        dataset.addSeries(rootPlot);
        JFreeChart chart = ChartFactory.createXYLineChart(
                "Graph for the Log function, using Secant Method",
                "x",
                "f(x)",
                dataset,
                PlotOrientation.VERTICAL,
