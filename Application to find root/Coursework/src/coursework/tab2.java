...
public class tab2 extends JFrame implements ActionListener {
    ...
    String[] columnNamesForBisection = {"Iteration", "xlower", "xupper", "xmiddle", "f(xlower)", "f(xupper)", "approx root"};
    ...
    double x1, x2, x3, startP1, startP2, root;
    int decimalPl, maxIt;
    String methodName;
    ...
    private void specifyPanel() // method for adding components that are not generic
    {
        JLabel l_function1 = new JLabel("f(x)=");
        JLabel l_function2 = new JLabel("x²+");
        JLabel l_function3 = new JLabel("x+");
        ...
        l_function3.setFont(bigfnt);
        l_function3.setAlignmentX(Component.CENTER_ALIGNMENT);

        inputA.setPreferredSize(new Dimension(40, 30));
        inputB.setPreferredSize(new Dimension(40, 30));
        inputC.setPreferredSize(new Dimension(40, 30));

        inputPanel.add(l_function1);
        inputPanel.add(inputA);
        inputPanel.add(l_function2);
        inputPanel.add(inputB);
        inputPanel.add(l_function3);
    ...
    public void actionPerformed(ActionEvent e) {
        if ("find".equals(e.getActionCommand())) {
            try // tries to parse the data user has input         
            {
                ...
                x2 = Double.parseDouble(inputB.getText());
                x3 = Double.parseDouble(inputC.getText());
                ...
                if (methodName.equals("Newton-Raphson")) {
                    solve(methodName, x1, x2, x3, startP1, decimalPl, maxIt, -1);
                } else {
                    startP2 = Double.parseDouble(startP2Field.getText());
                    solve(methodName, x1, x2, x3, startP1, decimalPl, maxIt, startP2);
                }
    ...
    private void solve(String methodName, double a, double b, double c, double sP1, int dp, int it, double sP2) {
        if (methodName.equals("Bisection")) {
            BisectionMethod BM = new BisectionMethod(a, b, c, sP1, sP2, dp, it);
            double[][] result = BM.getResult();
            root = BM.getRoot();
            displayBisectionData(result, root);
        }
        if (methodName.equals("Newton-Raphson")) {
            NewtonRhapsonMethod NM = new NewtonRhapsonMethod(a, b, c, sP1, dp, it);
        ...
        if (methodName.equals("Secant")) {
            SecantMethod SM = new SecantMethod(a, b, c, sP1, sP2, dp, it);
    ...
    private void displayBisectionData(double[][] result, double root) {
        ...
        JFreeChart chart = ChartFactory.createXYLineChart(
                "Graph for the Quadratic function, using Bisection Method",
                "x",
                "f(x)",
                dataset,
    ...
    private void displayNewtonData(double[][] result, double root) {
        tablePanel.removeAll(); // removes components in panel each time "solve" button is pressed so new table could be displayed
        graphPanel.removeAll(); // removes components in panel each time "solve" button is pressed so new graph could be displayed
        rootPanel.removeAll();  // removes components from panel each time "solve" button is pressed so new root could be displayed
        JLabel rootStr = new JLabel(Double.toString(root)); //converts root's value to string so it can be added to Jlabel;
        ...
        JFreeChart chart = ChartFactory.createXYLineChart(
                "Graph for the Quadratic function, using Newton-Raphson Method",
                "x",
                "f(x)",
                dataset,
                PlotOrientation.VERTICAL,
    ...
    private void displaySecantData(double[][] result, double root) {
        ...
        JFreeChart chart = ChartFactory.createXYLineChart(
                "Graph for the Quadratic function, using Secant Method",
                "x",
                "f(x)",
                dataset,
                PlotOrientation.VERTICAL,
    ...
    private void buttonvalidation() // method that adds DocumentListener to all text fields for later checking 
    {
        ...
        listOfFields.add(inputB);
        listOfFields.add(inputC);
